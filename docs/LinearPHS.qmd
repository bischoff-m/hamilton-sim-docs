...
An input-state-output port-Hamiltonian system (ISOPHS) with $n$-dimensional state space manifold $\mathcal{X}$, input and output spaces $U=Y=\R^m$, and Hamiltonian $H:\mathcal{X}\to\R$, is given by
\begin{align*}
\dot{x} &= [J(x) - R(x)]\nabla H(x) + g(x)u\\
y &= g^T(x) \nabla H(x)
\end{align*}
